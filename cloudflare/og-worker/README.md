...
| URL Pattern | OG Tags |
|-------------|---------|
| `/listings/:id` | Title, price, first image, seller name, condition |
| `/users/:id` | Display name, avatar, rating, review count, location |
| `/search?q=...` | Search query in title + description |
| `/*` (other) | Default DeelMarkt branding |
...
# Test: curl -H "User-Agent: WhatsApp" http://localhost:8787/listings/test-id
```
