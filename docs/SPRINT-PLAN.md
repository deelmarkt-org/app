...
- [x] `P-31-wire` Wire `PriceTag` into `DeelCard`, `DetailInfoSection`, `ListingCard`; add `originalPriceInCents` to `ListingEntity` ✅ PR #99
  - ⚠ **Deferred `[R]`:** `listings.original_price_cents` DB column not yet created. DTO field is forward-compatible (nullable). Tracked via [#100](https://github.com/deelmarkt-org/app/issues/100) item 3 + ADR-023 Q5. reso to open migration ticket when discount feature is scoped.
