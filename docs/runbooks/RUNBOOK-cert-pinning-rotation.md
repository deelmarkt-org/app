...
3. **Three parallel tracks:**
   - **Track A — App release (primary, always engage):** Update `network_security_config.xml` with the new pin (drop old pin if it's actively rejecting); expedited TestFlight + Play Internal → production. Practically the ONLY recovery path that scales to all affected users; B and C are accelerators, not replacements.
   - **Track B — Backstop pin disable (last resort, severe limitations):** Ship a Remote Config flag `cert_pinning_enabled` (default `true`); on emergency, set `false` to bypass pinning. **Significant caveats per gemini PR #264 review:**
     - **Chicken-and-egg:** Remote Config is itself fetched over TLS. If handshakes are already failing for `firebaseremoteconfig.googleapis.com`, the app cannot fetch the bypass flag. Track B only helps users who have already cached the flag (i.e. who fetched a fresher Remote Config recently) OR if the Remote Config endpoint happens to be on an unaffected CA chain.
     - **Native config cannot be bypassed at runtime:** Android's `network_security_config.xml` is parsed at app build time and applied by the OS-level network stack. A Remote Config flag cannot disable it without **custom networking code** that explicitly opts out of the system-level configuration (e.g. a custom OkHttp `TrustManager` or per-request override). DeelMarkt does not currently have such code; shipping it is itself a Track A release.
     - **Net implication:** Track B is realistic only as a future capability — when DeelMarkt ships a custom networking layer with a Remote-Config-controlled fallback. Until then, Track A + Track C are the only effective levers.
     - **Historical bypass risk:** even when Track B becomes shippable, pinning bypass means any compromised CA in the chain becomes exploitable. Use only when handshake failures are blocking critical flows (login, payment) and Track A is > 24h away. Bypass window must be documented for GDPR Art. 32 review.
   - **Track C — Pre-emptive force-update:** Set Remote Config `min_supported_app_version` to a build that includes the new pin; users on older builds get a non-dismissible update prompt. Same Remote Config dependency as Track B — works only if the user can fetch Remote Config, which requires successful handshakes elsewhere.
